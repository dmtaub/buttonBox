...
default_clearance = .27;//maker bot .33; // 
//clear_test = [ .15, .175, .2, .225, .25, .275];
clear_test = [ .00, .325, .3, .275, .25, .225];
...
//percentage of the radius of the snap sphere, to translate inward
...
default_font= "Proxima Nova:style=Semibold";
default_size= 3;
default_rotate_bool=false;

clear_test = [ .29, .29, .29,.29, .29, .29];
indices = ["1", "2","3","4","5","6"];

default_font="Arial:style=bold";
default_size = 10;
default_rotate_bool=true;
default_text = clear_test;

// print clearances on dice:
test_clearances(3, 3.5);
...
module test_clearances(sides=4, thick = 2, faces = default_text, font = default_font, size = default_size, rotate45 = default_rotate_bool, font_trans = [0,0,0.01]){
    hole = 12;
    high= 15;
    for ( i = [0 : len(clear_test)-1]){
        translate([(i%wrap)*spacing,-floor(i/wrap)*spacing,0])
            difference(){
                rotate(45)full_tile(sides,thick,hole,high,iter=i);
                rotate(rotate45 ? 45-90 : 0)
	...
	difference(){
		union(){
            ...
            *tweener( num_sides, poly_radius-.3, thick, button_rad, stellation_height, waypoint);
			...
			//make the snaps
			snap_maker(num_sides,radius,thick,snapwidth, iter);
		}
		union(){
            if (fillet > 0)
               fillet_maker(num_sides,radius,thick,snapwidth, fillet == 2, iter);
      	}
	}
}
...
module snap_maker(num_sides, radius, thick,snapwidth, iter){
    theta  =360/num_sides;
		...
		rotate(i*theta,[0,0,1]) 	
			...
			for(i=[0:snaps-1]){	
				...
				translate([radius,0,-thick/2]) 
					...
					translate([-lengthen-thick/2,2*(i+.5)*snapwidth+clearance(iter)/2,0]) 
						union(){
							hinge_a(thick/2+lengthen,snapwidth-clearance(iter),thick/2,.01,i);
						}
			}
 		}
	}
...
module fillet_maker(num_sides,radius, thick, snapwidth, both = true, iter){
			...
			rotate(180/num_sides,[0,0,1]) 
				translate([-angleclear,0,angleclear])
					translate([-thick/2,2*(snaps)*snapwidth,0]) 
						union(){
							translate([0,snapwidth-clearance(iter)/2,-angleclear])
								rotate([0,45,0])cube([angleclear*2,snapwidth+clearance(iter),angleclear*2]);
							if(both)
                                translate([0,snapwidth-clearance(iter)/2,thick-angleclear])
                                    rotate([0,45,0])cube([angleclear*2,snapwidth+clearance(iter),angleclear*2]);
						}
		//rotation is around the z-axis [0,0,1]
		rotate(j*360/num_sides,[0,0,1]) 	
